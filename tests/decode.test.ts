...
describe("final ABI account decoders", () => {
  it("decodes the 116-byte Market layout", () => {
    ...
    expect(market.priceAccount).toBeNull();
  });

  it("decodes the 158-byte Market with Hyperliquid price account", () => {
    ...
    Buffer.alloc(32, 7).copy(data, 116);
    data.writeUInt16LE(19000, 148);
    data.writeBigUInt64LE(10_000_000_000n, 150);

    const market = decodeMarket(data);
    expect(market.priceAccount).toEqual(Buffer.alloc(32, 7));
    expect(market.payoutBps).toBe(19000);
    expect(market.maxOpenExposure).toBe(10_000_000_000n);
  });

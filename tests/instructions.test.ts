...
describe("final ABI write builders", () => {
  ...
  it("encodes open_position arguments and exact account count", () => {
    const keys = Array.from({ length: 14 }, (_, index) => new PublicKey(Uint8Array.from({ length: 32 }, () => index + 1)));
    ...
    expect(instruction.keys[14]).toMatchObject({
      pubkey: crankSignerPda(keys[4]),
      isSigner: false,
      isWritable: false,
    });
    expect(instruction.keys[16]).toMatchObject({ pubkey: keys[2], isSigner: false, isWritable: false });

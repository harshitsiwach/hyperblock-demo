...
export interface DecodedMarket {
  ...
  priceAccount: Uint8Array | null;
  payoutBps: number | null;
  maxOpenExposure: bigint | null;
}

export interface DecodedPriceAccount {
...
export function decodeMarket(data: Buffer): DecodedMarket {
  ...
  const base: DecodedMarket = {
    ...
    priceAccount: null,
    payoutBps: null,
    maxOpenExposure: null,
  };
  if (data.length >= 158) {
    base.priceAccount = data.subarray(116, 148);
    base.payoutBps = data.readUInt16LE(148);
    base.maxOpenExposure = data.readBigUInt64LE(150);
  }
  return base;
}

export function decodePriceAccount(data: Buffer): DecodedPriceAccount {
  assertAccount(data, "PriceAccount", 81);
  const authority = data.subarray(8, 40);
  const symbolBytes = data.subarray(40, 56);
  const dexBytes = data.subarray(56, 64);
  const price = data.readBigUInt64LE(64);
  const timestamp = data.readBigInt64LE(72);
  const bump = data.readUInt8(80);
  const symbol = symbolBytes.toString("utf8").replace(/\0+$/, "");
  const dex = dexBytes.toString("utf8").replace(/\0+$/, "");
  return { authority, assetSymbol: symbol, dex, price, timestamp, bump };
}

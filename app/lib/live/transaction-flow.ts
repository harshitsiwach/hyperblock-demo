...
export async function prepareOpenPosition(
  ...
  const symbolForMarket: Record<number, string> = Object.fromEntries(MARKETS.map((m) => [m.marketId, m.symbol])) as Record<number, string>;
  const assetSymbol = symbolForMarket[config.marketId] ?? "BTC";
  const priceAccount = priceAccountPda(config.programId, assetSymbol);
  ...
  try {
    ...
    const instruction = openPositionInstruction(
      prepared.programId,
      {
        ...
        priceAccount: prepared.priceAccount,
      },
      {
        nonce: quote.nextPositionNonce,
        taskSalt,

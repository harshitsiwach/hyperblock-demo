...
pub fn handler(ctx: Context<SettlePosition>, nonce: u32, task_salt: [u8; 32]) -> Result<()> {
    ...
    let (user_payout, protocol_fee, lp_fee, outcome) = if is_refund {
    ...
    } else if ctx.accounts.market.price_account != Pubkey::default() {
        let settle_price_opt = try_read_price_account_for_settle(
        ...
        let settle_price = match settle_price_opt {
            Some(p) => p,
            None => return Ok(()),
        };
        // tie (open==close) is breakeven: user gets stake back - handler below via calculate_settlement returns collateral
        ...
        let outcome = if settle_price == position.entry_price {
            PositionOutcome::Breakeven
        } else {
            position_outcome(false, amounts.gross_profit, amounts.loss)
        };
        (
            amounts.user_payout,

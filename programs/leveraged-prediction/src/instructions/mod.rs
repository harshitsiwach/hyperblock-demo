...
use crate::state::{
    CompactPosition, Direction, Market, MarketMode, PriceAccount, ProtocolConfig, UserLiquidity,
    UserPositions,
};
use crate::{
    ...
    DEFAULT_MAX_OPEN_EXPOSURE, DEFAULT_PAYOUT_BPS, FEE_AUTHORITY_SEED, LEVERAGE, MARKET_SEED,
    MAX_MARKET_EQUITY, MAX_POSITION_COLLATERAL, MAX_PRICE_AGE_SECONDS, MIN_MARKET_EQUITY,
    MIN_POSITION_COLLATERAL, ORACLE_EXPONENT, ORACLE_MAX_AGE_SECONDS, ORACLE_MAX_CONFIDENCE_BPS,
    ORACLE_PROGRAM_ID, POSITION_DURATION_SECONDS, PRICE_SEED, PROFIT_FEE_BPS,
...
pub mod delegate_price_account;
pub mod delegate_user_liquidity;
pub mod delegate_user_positions;
pub mod deposit_liquidity;
pub mod execute_withdrawal;
pub mod initialize_market;
pub mod initialize_market_with_price_account;
pub mod initialize_price_account;
...
pub use delegate_price_account::DelegatePriceAccount;
...
pub use initialize_market_with_price_account::InitializeMarketWithPriceAccount;
pub use initialize_price_account::InitializePriceAccount;
...
pub(crate) fn price_to_i64(price_u64: u64) -> Result<i64> {
    // PriceAccount stores 1e6, on-chain position uses 1e8 (ORACLE_EXPONENT 8). Scale ×100.
    let scaled = (price_u64 as u128)
        .checked_mul(100)
        .ok_or(ErrorCode::MathOverflow)?;
    i64::try_from(scaled).map_err(|_| ErrorCode::MathOverflow.into())
}

pub(crate) fn read_price_account_unchecked(
    account: &UncheckedAccount,
    clock: &Clock,
    expected_key: Pubkey,
) -> Result<i64> {
    require_keys_eq!(account.key(), expected_key, ErrorCode::WrongPriceFeed);
    let data = account.try_borrow_data()?;
    let mut slice: &[u8] = &data;
    let price_account = PriceAccount::try_deserialize(&mut slice)?;
    require!(price_account.price != 0, ErrorCode::InvalidPrice);
    let age = clock
        .unix_timestamp
        .checked_sub(price_account.timestamp)
        .ok_or(ErrorCode::MathOverflow)?;
    require!(age <= MAX_PRICE_AGE_SECONDS, ErrorCode::PriceTooStale);
    require!(age >= 0, ErrorCode::PriceTooStale);
    price_to_i64(price_account.price)
}

pub(crate) fn try_read_price_account_for_settle(
    account: &UncheckedAccount,
    clock: &Clock,
    expected_key: Pubkey,
    expires_at: i64,
    ...
    require_keys_eq!(account.key(), expected_key, ErrorCode::WrongPriceFeed);
    let data = account.try_borrow_data()?;
    if data.len() < 8 + PriceAccount::INIT_SPACE {
        msg!("price account data too short; no-op");
        return Ok(None);
    }
    let mut slice: &[u8] = &data;
    let price_account = match PriceAccount::try_deserialize(&mut slice) {
        Ok(acc) => acc,
        Err(_) => {
            msg!("price account deserialize failed; no-op");
            return Ok(None);
        }
    };
    if price_account.price == 0 {
        msg!("price is zero; no-op");
        return Ok(None);
    }
    let age = match clock.unix_timestamp.checked_sub(price_account.timestamp) {
        Some(a) => a,
        None => {
            msg!("invalid timestamp; no-op");
            return Ok(None);
        }
    };
    if age > MAX_PRICE_AGE_SECONDS || age < 0 {
        msg!("price too stale; no-op");
        return Ok(None);
    }
    if price_account.timestamp < expires_at || price_account.timestamp >= refund_at {
        msg!("price sample outside settlement interval; no-op");
        return Ok(None);
    }
    Ok(Some(price_to_i64(price_account.price)?))
}

...
pub fn handler(
    ...
    // Hyperliquid path: Market.price_account is canonical. Fall back to legacy Pyth for old markets.
    let entry_price: i64 = if ctx.accounts.market.price_account != Pubkey::default() {
        let p = read_price_account_unchecked(
            &ctx.accounts.price_account,
            &clock,
            ctx.accounts.market.price_account,
        )?;
        require!(
            p >= min_entry_price && p <= max_entry_price,
    ...
    // per-market max_open_exposure cap (0 = uncapped for legacy)
    if ctx.accounts.market.max_open_exposure != 0 {
        require!(
            open_collateral_after <= ctx.accounts.market.max_open_exposure,
            ErrorCode::RiskLimitExceeded
        );
    }

    // Build settlement instruction with both price feeds (hyperliquid + legacy)
    let (sched_price_update, sched_price_account) = if ctx.accounts.market.price_account
        != Pubkey::default()
    {
        (
        ...
        (ctx.accounts.price_update.key(), Pubkey::default())
    };
    let scheduled_instruction = settlement_instruction(
        crate::accounts::SettlePosition {
            ...
            price_update: sched_price_update,
            price_account: sched_price_account,
...
pub struct OpenPosition<'info> {
    ...
    /// CHECK: Hyperliquid price account; validated in handler when market.price_account is set.
...
mod tests {
    ...
    fn scheduled_settlement_uses_generated_account_order_and_writability() {
        let keys = (0..14).map(|_| Pubkey::new_unique()).collect::<Vec<_>>();
        let generated = crate::accounts::SettlePosition {
            ...
            price_account: keys[11],
            token_program: keys[12],
            crank_signer: keys[13],
        };
